...
Some tests for VoronoiSkeleton
...
public:
  ...
  void init(const cv::Mat1b & query,
    ...
    if (_crop_img_before)
      VoronoiSkeleton::copy_bounding_box_plusone(query, _first_img);
  ...
  cv::Mat1b current_skel() const {
    return _skel.get_skeleton().clone();
  }
  ...
  bool iter() {
    ...
    bool success = false;
    if (reuse)
      success = _skel.thin(_curr_skel, _implementation_name, false, 1); // already cropped
    else
      success = _skel.thin(_first_img, _implementation_name, false, _nframes); // already cropped
    _skel.get_skeleton().copyTo(_curr_skel);
    return success;
  }

  inline bool has_converged() const { return _skel.has_converged(); }
...
template<class T>
void paste_pics(const std::vector<cv::Mat_<T> > & in,
                cv::Mat_<T> & out,
  ...
  // prepare output
  int galleryrows = ceil(nimgs / gallerycols);
...
void generate_video_comparer(const cv::Mat1b & query,
  ...
  while(n_terminated < nimpls) {
    ...
    // now make collage
    paste_pics(curr_imgs, curr, gallerycols, cv::Vec3b(0, 0, 0), true, CV_RGB(255, 255, 255));
...
int CLI(int argc, char** argv) {
  ...
  if (order != VIDEO_COMPARER && order != BENCHMARK) {
    first_file_idx = 3;
    if (!VoronoiSkeleton::is_implementation_valid(implementation_name)) {
  ...
  if (order == THIN) {
    for (unsigned int file_idx = 0; file_idx < files.size(); ++file_idx) {
      Timer timer;
      bool ok = skel.thin(files[file_idx], implementation_name, true);
      if (!ok) {
      ...
      cv::imwrite(out.str(), skel.get_skeleton());
      printf("Written file '%s'\n", out.str().c_str());
      // show res
      cv::imshow("query", files[file_idx]);
      cv::imshow(implementation_name, skel.get_skeleton());
      cv::waitKey(0);
    } // end loop file_idx
  } // end if (order == THIN)
...
int main(int argc, char** argv) {
  ...
  //  cv::Mat1b out;
  //  ims.push_back(cv::imread(IMG_DIR "depth/juggling1_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE));
  //  ims.push_back(cv::imread(IMG_DIR "depth/juggling1_depth.png", CV_LOAD_IMAGE_GRAYSCALE));
  //  ims.push_back(cv::imread(IMG_DIR "depth/juggling2_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE));
  //  ims.push_back(cv::imread(IMG_DIR "depth/juggling2_depth.png", CV_LOAD_IMAGE_GRAYSCALE));
  //  paste_pics(ims, out, 2, (uchar) 0, true, CV_RGB(255, 255, 255));
